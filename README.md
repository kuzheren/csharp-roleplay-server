# Proton Network Engine and Role Play Server

Мой Magnum Opus школьных времен (9 класс, 2023 год).

Проект представляет собой кастомный сетевой движок и реализацию Role Play сервера для SA:MP-подобной игры на Unity. Написан практически с нуля на C#.

Главной целью было не только создание игры, но и разработка собственного сетевого протокола, игрового мода, а также попытка разобраться в работе онлайн игр.

## Технологический стек

*   **Язык:** C#
*   **Движок:** Unity / .NET Console App (Сервер)
*   **Сеть:** LiteNetLib
*   **Бэкенд:** PHP + SQLite (доступ к базе данных через HTTP-запросы)

## Ключевые особенности

### 1. Бинарный сериализатор (`ProtonStream`)
Вместо использования стандартных решений (JSON или встроенных сериализаторов C#), я сделал собственную систему бинарной сериализации для минимизации размера пакетов и попытки разобраться в сериализации самостоятельно.
